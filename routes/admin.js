...
router.get('/view-all-projects', async (req, res, next) => {
    ...
    Project.find()
        .populate('mainHandler')
        .populate('subHandler')
        .then((projects) => {
            for (let i = 0; i < projects.length; i++) {
                projectsChunks.push(projects[i]);
            }
            res.render('Admin/viewAllProjects', {
                title: 'All Projects',
                csrfToken: req.csrfToken(),
                projects: projectsChunks,
                userName: req.session.user.name
            })  
        })
})
 ...
 */
router.get('/add-project', async function addProject(req, res, next) {
 ...
 * Known Bugs: None
 */
router.post('/store-project', async (req, res) => {
    const formData = {...req.body};
    console.log(formData)
    mainHandler = await User.findOne({name: formData.mainHandler})
    console.log(mainHandler)
    subHandler = await User.findOne({name: formData.subHandler})
    console.log(subHandler)
    delete formData.mainHandler
    delete formData.subHandler
    formData.mainHandler = mainHandler._id
    formData.subHandler = subHandler._id
    const project = new Project(formData)
    try {
        await project.save(formData)
    } catch(err) {
        console.log(err)
    }
